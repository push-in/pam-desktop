...
CURRENT_VERSION=$(sed -n "s/^const HOST_VERSION = '\([^']*\)';$/\1/p" \
    "$ROOT/packages/desktop/bin/pam-desktop")
[ -n "$CURRENT_VERSION" ]

for version in 0.9.0 1.0.0 1.1.0 1.2.1 1.2.5 1.2.7 "$CURRENT_VERSION"
do
